		...
		<form name="form_category_create" id="form_category_create" method="post" enctype='application/x-www-form-urlencoded' accept-charset="UTF-8" class="form">
			...
			if (isset($success)) {
				if (!$success) {
					...
					print "</ul>";
				} else print "<p class='success'>" . $current_group_page['name'] . " created succesfuly.<span class='note'>( id: <a href='category-edit.php?id=$last' target='_blank'>$last</a> )</span></p>";
			}
			?>
			...
			<select name='parent'>
				...
				<?php foreach( $categories as $row ) print "<option value='" . $row['id'] . "' " . ( ( isset( $p_parent ) && ! $success && $row['id'] == $p_parent) ? "selected='true'" : '') . ">" . $row['name'] . "</option>"; ?>
			</select>
			<br />
			...
			<span><input type='submit' name='category_slug_generate' value='Generate'/></span>
			<p class='note'>Required</p>
			<br />

			<label for='category_create'></label>
			<input type='submit' name='category_create' value='Create'/>
